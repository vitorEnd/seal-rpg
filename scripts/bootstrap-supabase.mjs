...
async function importSnapshot(supabase, snapshot) {
  ...
  await upsertRows(
    ...
    snapshot.characterClassOptions.map((option) => ({
      ...
      bonus_physical: option.bonusAttributes.physical,
      bonus_agility: option.bonusAttributes.agility,
      bonus_marksmanship: option.bonusAttributes.marksmanship,
      bonus_perception: option.bonusAttributes.perception,
      bonus_technique: option.bonusAttributes.technique,
      bonus_control: option.bonusAttributes.control,
      sort_order: option.order,
      active: option.active,
      ...toSnakeTimestampFields(option),
    })),
